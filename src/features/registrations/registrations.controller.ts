...
export const submitRegistration: RequestHandler = async (request, response, next) => {
  try {
    ...
    if (!flow || flow.status !== 'published' || !flow.publishedVersion) throw new HttpError(409, 'Registration is not currently open.');
    ...
    for (const field of fields) {
      ...
      if (field.required && (value === undefined || value === null || value === '')) throw new HttpError(400, `${field.label} is required.`);
    }
    ...
    const registration = await db.transaction(async (transaction) => {
      ...
      if (Object.keys(profileChanges).length) await transaction.update(users).set({ ...profileChanges, updatedAt: new Date() }).where(eq(users.id, request.auth!.user.id));
      return created;
    });

...
export const requireAuthentication: RequestHandler = async (request, _response, next) => {
  try {
    ...
    if (!auth) throw new HttpError(401, 'Your session has expired. Please sign in again.');
    request.auth = auth;
    next();
  } catch (error) {
    next(error);
...
export const requireInstitutionRole = (...roles: InstitutionRole[]): RequestHandler => (request, _response, next) => {
...
export const getBearerToken = (authorization?: string) => bearerToken(authorization);

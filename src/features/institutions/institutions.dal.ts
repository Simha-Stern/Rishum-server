...
const writeAuditLog = (
  ...
  db
    .insert(auditLogs)
    .values({ actorUserId, action, entityType, entityId, institutionId });
...
export const institutionsDal = {
  findAll: () =>
    db
      ...
      .innerJoin(
        registrationFlows,
        eq(registrationFlows.institutionId, institutions.id),
      )
      .where(
        and(
  ...
  async createInstitution(
    ...
    return db.transaction(async (transaction) => {
      const {
        ...
        secretaryEmail,
        actorUserId,
        registrationFlowName: _registrationFlowName,
        ...details
      } = input;
      ...
      const grantOrInvite = async (email: string, role: InstitutionRole) => {
        ...
        if (user) {
          await transaction
            ...
            .values({ institutionId: created.id, userId: user.id, role });
          return;
        }
  ...
  async grantOrInviteMember(
    ...
    if (user) {
      await db
        .insert(institutionMemberships)
        .values({ institutionId, userId: user.id, role })
        .onConflictDoUpdate({
          target: [
            institutionMemberships.userId,
    ...
    const [invitation] = await db
      ...
      .onConflictDoUpdate({
        target: [
          institutionInvitations.institutionId,
          institutionInvitations.invitedEmail,
        ],
  ...
  async findTeamMemberByEmail(
    ...
    const [activeMember] = await db
      .select({ entityId: users.id, role: institutionMemberships.role })
    ...
    const [pendingMember] = await db
      .select({
        entityId: institutionInvitations.id,
  ...
  countActiveManagers: async (institutionId: string) => {
    const [result] = await db
      ...
      .where(
        and(
          eq(institutionMemberships.institutionId, institutionId),
          eq(institutionMemberships.role, "manager"),
        ),
      );
    return result.total;
  },
  ...
  async listInstitutionTeam(institutionId: string) {
    ...
    const pendingMembers = await db
      ...
      .from(institutionInvitations)
      .where(eq(institutionInvitations.institutionId, institutionId));
    return [
